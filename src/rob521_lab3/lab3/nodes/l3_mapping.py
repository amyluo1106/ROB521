...
class OccupancyGripMap:
    ...
    def scan_cb(self, scan_msg):
        ...
        x_start = (odom_map[0]-self.map_msg.info.origin.position.x-MAP_DIM[0]/2)/CELL_SIZE
        y_start = (odom_map[1]-self.map_msg.info.origin.position.y-MAP_DIM[1]/2)/CELL_SIZE
        for i in range(len(scan_msg.ranges)):
            if scan_msg.ranges[i] == np.inf:
                continue
            # Range measurement in pixels
            range_mes = scan_msg.ranges[i]/CELL_SIZE
    ...
    def ray_trace_update(self, map, log_odds, x_start, y_start, angle, range_mes):
        ...
        rr, cc = ray_trace(int(round(y_start)), int(round(x_start)), int(round(y_end)), int(round(x_end)))
        ray = np.column_stack([rr, cc])
        
        for i in range(len(ray)):
            if ray[i][0] > map.shape[0]-1 or ray[i][1] > map.shape[1]-1 or ray[i][0] < 0 or ray[i][1] < 1:
                # outside map
                break
            if i < ray.shape[0]-NUM_PTS_OBSTACLE:
                # -beta if vell in FOV and closer than measured range
                log_odds[ray[i][0], ray[i][1]] -= BETA
            else:
                # alpha if cell in FOV and near measured range
                log_odds[ray[i][0], ray[i][1]] += ALPHA
            p = np.exp(log_odds[ray[i][0], ray[i][1]])/(1+np.exp(log_odds[ray[i][0], ray[i][1]]))
            map[ray[i][0], ray[i][1]] = np.round(p*100).astype(np.uint8)
